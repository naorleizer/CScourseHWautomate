...
def diff_the_merge(diff_loc, source_file, test_file):
    ...
    os.execv(
        diff_loc.resolve(),
        [
            f'"{source_file.resolve()}"',   # not sure why, but the first argutment is ignored, so placing it twice works... -_-
...
def input_file_handling(input_file, ending):
    ...
    (
        print(
            f"File {colorama.Fore.RED}{input_file.name}{colorama.Fore.RESET} does not exists"
        )
        & exit()
...
def main():
    colorama.init()  # init colorama coloring engine
    ...
    DIFFMERGE_FLAG = True   # changes to False with a command line argument
    ...
    if len(in_file_list) != len(out_file_list):
        print(
            f"{colorama.Fore.RED}Differet amount of input ({len(in_file_list)}) and output ({len(out_file_list)}) files, please fix the issue{colorama.Fore.RESET}{os.linesep}Input files:{os.linesep}{' ,'.join([x.name for x in in_file_list])}{os.linesep}Output files:{os.linesep}{' ,'.join([x.name for x in out_file_list])}"
        )
        exit()
    ...
    for output_num, cur_in_file in enumerate(in_file_list):
        ...
        command = f'python "{pyfile.absolute()}" < "{cur_in_file.absolute()}" > "{out_file.absolute()}"'
        ...
        for a, b, line_num in zip(
            ...
            for c1, c2 in zip(a, b):
                if c1 == c2:
                    ...
                    parse1 += colorama.Fore.RED + c1 + colorama.Fore.RESET
                    parse2 += colorama.Fore.RED + c2 + colorama.Fore.RESET
                    zero_issues = False
                    current_file_all_clear = False
        if current_file_all_clear:
            print(
                f"👍 -All good with test {colorama.Fore.GREEN}{output_num+1}:{cur_in_file.name}{colorama.Fore.RESET} ✔"
            )
        else:
            print(
    ...
    print(
        f"❌ - Issues found with {colorama.Fore.RED}{', '.join(problem_file_list)}{colorama.Fore.RESET} ❌"
